...
// Hostmask is a mask that can be applied to IPv6 addresses to masks out bits
...
// ErrAddressOutOfRange will be returned. If the hostmask is present but set
// to 0 an ErrBadMaskLength will be returned (use DecrementIP6By() instead)
...
// ErrAddressOutOfRange will be returned. If the hostmask is present but set
// to 0 an ErrBadMaskLength will be returned (use IncrementIP6By() instead)
...
// mergeBitMasks merges two bitmasks by stacking their 1's bits to the left
// side. It will return an error if there are more than 8 total 1's bits
// between the two bytes
func mergeBitMasks(b1, b2 byte) (byte, error) {
	m := []byte{ 0x00, 0x80, 0xc0, 0xe0, 0xf0, 0xf0, 0xf8, 0xfc, 0xfe, 0xff }

	zeroes := bits.TrailingZeros8(b1) + bits.TrailingZeros8(b2)
	if zeroes < 8 {
		return byte(0), ErrBadMaskLength
	}
	return m[8 - zeroes], nil
}